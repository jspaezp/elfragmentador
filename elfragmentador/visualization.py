...
def make_bipartite(x):
    ...
    B = nx.Graph()
    B.add_nodes_from([x  for x in x.index], bipartite=0) # Add the node attribute "bipartite"
    B.add_nodes_from([x + "_" for x in x.index], bipartite=1)
    
    for index1 in x.index:
        for index2 in x.index:
            B.add_edges_from([(index1, index2 + '_'), ], weight = x[index1].loc[index2])
...
def plot_bipartite_seq(B):
    ...
    """
    if plt is None:
        raise ImportError("Matplotlib is not installed, please install and re-load elfragmentador")
    ...
    nx.draw(B, pos=pos, edge_color=weights, edge_cmap=plt.cm.Blues, with_labels=True, width=3, alpha=0.5)
    plt.show()
