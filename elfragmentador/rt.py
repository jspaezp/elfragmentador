...
def calculate_file_iRT(file: Union[Path, str]) -> pd.DataFrame:
    ...
    for g, sub_df in df.groupby("File Name"):
        ...
        irt_sub_df["iRT"] = [constants.IRT_PEPTIDES[x]['irt'] for x in irt_sub_df['Peptide Modified Sequence']]
        fit = polyfit(irt_sub_df["RT"], irt_sub_df["iRT"])
        fits.update({g:fit}) 

    pred_irt = lambda rt, poly: None if poly is None else rt * poly["polynomial"][0] + poly["polynomial"][1]
    df["Calculated iRT"] = [pred_irt(y, fits.get(x, None)) for x, y in zip(df["File Name"], df['RT'])]
